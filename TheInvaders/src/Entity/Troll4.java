...
public class Troll4 extends Entity{
    private int frame = 14;
    private int frameDelay = 0;
    boolean right=true;
    boolean left=false;
    boolean attack=false;
    private int x_min, y_min,x_max,y_max;
    public Troll4(int x, int y, int width, int height, boolean solid, Id id, Handler handler,
    ...
    public void draw(Graphics g) {
        ...
        if(isDeath()) {
            ...
            g.drawImage(EditImage.mirror(Game.troll4[frame-7].getBufferedImage()), getX(), getY(), width, height, null);
        }
        else if(attack) {
            g.drawImage(EditImage.mirror(Game.troll4[frame-14].getBufferedImage()), getX(), getY(), width, height, null);
        }
        else if(right==true)
            g.drawImage(Game.troll4[frame].getBufferedImage(), getX(), getY(), width, height, null);
        else
            g.drawImage(EditImage.mirror(Game.troll4[frame].getBufferedImage()), getX(), getY(), width, height, null);
    }
